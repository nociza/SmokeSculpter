...
(async function () {
  ...
  const SIMULATION_RESOLUTION = new THREE.Vector3(50, 50, 50);
  const GRID_SPACING = 0.005;
  const SIMULATION_SPACE = SIMULATION_RESOLUTION.multiplyScalar(GRID_SPACING);
  const CELL_NUM =
  ...
  const reset = function () {
    ...
    const addSmoke = function (deltaTime) {
      ...
      const heatSourceCenter = new THREE.Vector2(
        mousePosition.x / (canvas ? canvas.width : 1),
        mousePosition.y / (canvas ? canvas.height : 1)
      );
      gl.uniform2fv(
    ...
    const RENDER_SCALE = SIMULATION_SPACE.multiplyScalar(
      75.0 / SIMULATION_SPACE.y
    );
    const CAMERA_POSITION = new THREE.Vector3(100.0, 100.0, 150.0);
    const MODEL_MATRIX = new THREE.Matrix4();
    const iNV_MODEL_MATRIX = new THREE.Matrix4();
    const VIEW_MATRIX = new THREE.Matrix4()
      .lookAt(
        CAMERA_POSITION,
    ...
    const setMvpMatrix = function () {
      ...
      mvpMatrix = VIEW_MATRIX.multiply(projectionMatrix);
    };
    setMvpMatrix();
